...
static int cdx_get_extradata(AVCodecContext *avctx, CdxStreamT *pb, int size)
{
    ...
    if (ret != size) {
        ...
        CDX_LOGE("Failed to read extradata of size %d\n", size);
        return -1;
    }
...
static int cdx_mov_read_chan(CdxStreamT *pb, int64_t size)
{
    ...
    for (i = 0; i < num_descr; i++) {
        uint32_t label;
        if (CdxStreamEos(pb)) {
            CDX_LOGE("reached EOF while reading channel layout\n");
            return -1;
        }
...
got_sound:
    if (!impl->avctx.block_align) {
        CDX_LOGE("could not find COMM tag or invalid block_align value\n");
        goto OPENFAILURE;
    }
    ...
    impl->mErrno = PSR_OK;
    CDX_LOGE("offset : %lld, Snd data end : %lld, dSndataOffset : %lld",
        offset, aiff->data_end, impl->dSndataOffset);
    pthread_cond_signal(&impl->cond);
    return 0;
OPENFAILURE:
    CDX_LOGE("AiffOpenThread fail!!!");
...
static cdx_int32 __AiffParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    if(impl->aiff.data_end != 0)
    {
        ...
        {
            CDX_LOGE("AiffParser Prefetch to the data end!");
            return CDX_FAILURE;
        }
    }
...
static cdx_int32 __AiffParserSeekTo(CdxParserT *parser, cdx_int64 timeUs)
{
    ...
    {
        CDX_LOGE("invalid position (out of bound) : %lld, bound : %lld"
